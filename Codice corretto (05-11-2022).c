...
void dividi ()
{
        ...
        float divisione; //inserisco l'assegnazione della divisione
        int controllo_a;
        int controllo_b;
do{  //uso sempre il ciclo do-while per il controllo
        ...
        divisione = (float) a / (float) b;  //ho messo l'operatore / (divisione) al posto dell'operatore % (modulo) per ottenere una divisione con risultato decimale

        printf ("\n\nLa divisione tra %d e %d e': %f", a,b,divisione);
        }
        }while(controllo_a == 0 || controllo_b == 0);
}
